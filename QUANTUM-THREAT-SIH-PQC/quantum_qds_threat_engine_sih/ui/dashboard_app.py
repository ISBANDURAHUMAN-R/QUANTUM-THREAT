...
crypto_engine = CryptoReadinessEngine()
...
class DashboardRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed_path.path == '/api/crypto-assess':
            content_length = int(self.headers.get('Content-Length', 0))
            post_data = self.rfile.read(content_length).decode('utf-8')
            params = json.loads(post_data) if post_data else {}
            asset = CryptoAsset(
                name=params.get('name', 'demo-artifact.bin'),
                algorithm=params.get('algorithm', 'RSA'),
                key_size=int(params.get('key_size', 2048)),
                certificate_status=params.get('certificate_status', 'valid'),
                signature_status=params.get('signature_status', 'valid'),
                sensitive=bool(params.get('sensitive', True)),
                data_lifetime_years=int(params.get('data_lifetime_years', 10)),
                suspicious_activity=float(params.get('suspicious_activity', 0.0)),
                certificate_issuer_trusted=bool(params.get('certificate_issuer_trusted', True)),
            )
            payload = params.get('payload')
            result = crypto_engine.assess(asset, payload=payload)
