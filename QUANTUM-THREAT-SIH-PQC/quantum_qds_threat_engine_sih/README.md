...
## SIH Crypto Readiness & Post-Quantum Migration Layer

The project now extends beyond quantum-channel attack simulation into a practical cryptographic readiness workflow:

**DISCOVER → VERIFY → DETECT → ASSESS → SCORE → MIGRATE**

- SHA-256 payload hashing
- RSA/ECDSA classical signature risk assessment
- Certificate and signature status checks
- Suspicious-activity risk input
- Harvest-Now-Decrypt-Later exposure assessment
- Shor's-algorithm quantum exposure mapping for RSA/ECDSA
- Explainable 0–100 risk score
- ML-DSA primary migration recommendation
- SLH-DSA alternative recommendation
- Real-world incident mapping: DigiNotar, SolarWinds, HNDL, and RSA/ECC quantum risk

### API demo

```bash
curl -X POST http://localhost:8080/api/crypto-assess \\
  -H "Content-Type: application/json" \\
  -d '{"name":"company_update.exe","algorithm":"RSA","key_size":2048,"certificate_status":"valid","signature_status":"valid","sensitive":true,"data_lifetime_years":10,"suspicious_activity":0.82,"payload":"demo software artifact"}'
```

The response contains the SHA-256 digest, classical security state, quantum risk, risk score, reasons, incident mapping, and PQC migration recommendation.

> **Important:** this layer is a readiness/risk assessment and migration aid. It does not claim that a valid digital signature proves that software is safe; behavioral/integrity telemetry must be evaluated separately.
